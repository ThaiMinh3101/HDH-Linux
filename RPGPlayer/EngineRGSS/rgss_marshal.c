...
//      -4  ≤  n ≤ -1               → read (-n) little-endian bytes (sign-extend)
//       5  ≤  n ≤ 127              → value = n - 5
//      -128 ≤  n ≤ -6              → value = n + 5
//
//   Symbol (':', ';'):  ':' stores new symbol (length + bytes); ';' is sym_table index.
//   Object link ('@'):  references a previously decoded object by object-table index.
//   Instance variables ('I'): wraps another value, then appends a Hash of ivars.
//                             We decode the inner value and ignore encoding ivars.
...
static void *arena_alloc(RGSSArena *a, size_t sz) {
    ...
    if (a->used + aligned > a->cap) return NULL;  // out of space
...
static long long read_marshal_int(RGSSMarshalDecoder *d) {
    if (d->last_error != RGSS_MARSHAL_OK) return 0;
    int8_t n = (int8_t)read_byte(d);
    if (d->last_error != RGSS_MARSHAL_OK) return 0;

    if (n == 0) return 0;

    if (n > 4) return (long long)(n - 5);
    if (n < -4) return (long long)(n + 5);
...
static int intern_symbol(RGSSMarshalDecoder *d, char *sym) {
    if (d->sym_count >= d->sym_cap) {
        ...
        if (!t) { d->last_error = RGSS_MARSHAL_ERR_ALLOC; return -1; }
        d->sym_table = t;
        d->sym_cap   = new_cap;
    }
...
static RGSSValue *alloc_value(RGSSMarshalDecoder *d, RGSSValueType type) {
    ...
    if (!v) { d->last_error = RGSS_MARSHAL_ERR_ALLOC; return NULL; }
...
static RGSSValue *decode_string(RGSSMarshalDecoder *d) {
    ...
    if (d->last_error != RGSS_MARSHAL_OK) return NULL;
    if (len < 0 || len > 64 * 1024 * 1024) {
        d->last_error = RGSS_MARSHAL_ERR_OVERFLOW; return NULL;
    }
    ...
    if (!v->as.s.data && len > 0) { d->last_error = RGSS_MARSHAL_ERR_ALLOC; return NULL; }
    if (len > 0) memcpy(v->as.s.data, d->buf + d->pos, (size_t)len);
    d->pos += (size_t)len;
...
static char *decode_symbol_name(RGSSMarshalDecoder *d) {
    long long len = read_marshal_int(d);
    if (d->last_error != RGSS_MARSHAL_OK) return NULL;
    if (len <= 0 || len > 1024) { d->last_error = RGSS_MARSHAL_ERR_OVERFLOW; return NULL; }
    ensure_bytes(d, (size_t)len);
    if (d->last_error != RGSS_MARSHAL_OK) return NULL;

    char *sym = arena_strdup(d->arena, (const char *)(d->buf + d->pos), (size_t)len);
    d->pos += (size_t)len;
    if (!sym) { d->last_error = RGSS_MARSHAL_ERR_ALLOC; return NULL; }
    return sym;
}
...
static RGSSValue *decode_array(RGSSMarshalDecoder *d) {
    ...
    if (d->last_error != RGSS_MARSHAL_OK) return NULL;
    if (count < 0 || count > 1024 * 1024) { d->last_error = RGSS_MARSHAL_ERR_OVERFLOW; return NULL; }

    v->as.arr.count = (size_t)count;
    if (count > 0) {
        v->as.arr.items = arena_alloc(d->arena, (size_t)count * sizeof(RGSSValue *));
        if (!v->as.arr.items) { d->last_error = RGSS_MARSHAL_ERR_ALLOC; return NULL; }
...
static RGSSValue *decode_hash(RGSSMarshalDecoder *d) {
    ...
    long long count = read_marshal_int(d);
    if (d->last_error != RGSS_MARSHAL_OK) return NULL;
    if (count < 0 || count > 512 * 1024) { d->last_error = RGSS_MARSHAL_ERR_OVERFLOW; return NULL; }

    v->as.hash.count  = (size_t)count;
    v->as.hash.keys   = NULL;
    v->as.hash.values = NULL;
    if (count > 0) {
        v->as.hash.keys   = arena_alloc(d->arena, (size_t)count * sizeof(RGSSValue *));
        v->as.hash.values = arena_alloc(d->arena, (size_t)count * sizeof(RGSSValue *));
        if (!v->as.hash.keys || !v->as.hash.values) {
            d->last_error = RGSS_MARSHAL_ERR_ALLOC; return NULL;
        }
        for (long long i = 0; i < count; i++) {
...
static RGSSValue *decode_object(RGSSMarshalDecoder *d) {
    ...
    if (tag == ':') {
        ...
        if (idx < 0 || (size_t)idx >= d->sym_count) {
            d->last_error = RGSS_MARSHAL_ERR_TRUNCATED; return NULL;
        }
        class_name = d->sym_table[idx];
    } else {
        d->last_error = RGSS_MARSHAL_ERR_UNSUPPORTED; return NULL;
    }
    ...
    if (d->last_error != RGSS_MARSHAL_OK) return NULL;
    if (icount < 0 || icount > 512 * 1024) { d->last_error = RGSS_MARSHAL_ERR_OVERFLOW; return NULL; }

    v->as.obj.ivars.count  = (size_t)icount;
    v->as.obj.ivars.keys   = NULL;
    v->as.obj.ivars.values = NULL;
    if (icount > 0) {
        v->as.obj.ivars.keys   = arena_alloc(d->arena, (size_t)icount * sizeof(RGSSValue *));
        v->as.obj.ivars.values = arena_alloc(d->arena, (size_t)icount * sizeof(RGSSValue *));
...
static RGSSValue *decode_value(RGSSMarshalDecoder *d) {
    ...
    switch (tag) {
        ...
        case 'f': {
            ...
            long long len = read_marshal_int(d);
            if (d->last_error != RGSS_MARSHAL_OK) return NULL;
            if (len <= 0 || len > 64) { d->last_error = RGSS_MARSHAL_ERR_OVERFLOW; return NULL; }
            ensure_bytes(d, (size_t)len);
            if (d->last_error != RGSS_MARSHAL_OK) return NULL;
            char buf[65];
            ...
            if (v) {
                if (strcmp(buf, "inf") == 0) v->as.f = HUGE_VAL;
                else if (strcmp(buf, "-inf") == 0) v->as.f = -HUGE_VAL;
                else if (strcmp(buf, "nan") == 0) v->as.f = 0.0 / 0.0;
                else v->as.f = strtod(buf, NULL);
            }
            register_object(d, v);
            return v;
        }
...
RGSSValue *rgss_marshal_load(RGSSMarshalDecoder *d) {
    ...
    if (d->len < 2) { d->last_error = RGSS_MARSHAL_ERR_TRUNCATED; return NULL; }
    if (d->buf[0] != 0x04 || d->buf[1] != 0x08) {
        d->last_error = RGSS_MARSHAL_ERR_VERSION; return NULL;
    }
    d->pos = 2;
...
static int enc_ensure(RGSSMarshalEncoder *e, size_t extra) {
    ...
    while (new_cap < e->len + extra) new_cap *= 2;
    uint8_t *nb = realloc(e->buf, new_cap);
    if (!nb) { e->last_error = RGSS_MARSHAL_ERR_ALLOC; return 0; }
    e->buf = nb;
    e->cap = new_cap;
    return 1;
}
...
static void enc_int(RGSSMarshalEncoder *e, long long n) {
    if (n == 0) { enc_byte(e, 0); return; }
    if (n > 0 && n <= 122) { enc_byte(e, (uint8_t)(n + 5)); return; }
    if (n < 0 && n >= -123) { enc_byte(e, (uint8_t)(n - 5)); return; }
    ...
    if (n > 0) {
        ...
        if      (n >= -(1LL <<  7)) bytes = 1;
        else if (n >= -(1LL << 15)) bytes = 2;
        else if (n >= -(1LL << 23)) bytes = 3;
        else                        bytes = 4;
        enc_byte(e, (uint8_t)(-bytes));
        for (int i = 0; i < bytes; i++) {
            enc_byte(e, (uint8_t)(n & 0xFF));
...
static int enc_find_sym(RGSSMarshalEncoder *e, const char *sym) {
    for (size_t i = 0; i < e->sym_count; i++) {
        if (strcmp(e->sym_table[i], sym) == 0) return (int)i;
    }
    return -1;
}
...
static int enc_intern_sym(RGSSMarshalEncoder *e, const char *sym) {
    if (e->sym_count >= e->sym_cap) {
        ...
        if (!t) { e->last_error = RGSS_MARSHAL_ERR_ALLOC; return -1; }
        e->sym_table = t;
        e->sym_cap   = nc;
    }
...
static int enc_find_obj(RGSSMarshalEncoder *e, const void *ptr) {
    for (size_t i = 0; i < e->obj_count; i++) {
        if (e->obj_table[i] == ptr) return (int)i;
    }
    return -1;
}
...
static void enc_register_obj(RGSSMarshalEncoder *e, const void *ptr) {
    if (e->obj_count >= e->obj_cap) {
        ...
        if (!t) { e->last_error = RGSS_MARSHAL_ERR_ALLOC; return; }
        e->obj_table = t;
        e->obj_cap   = nc;
    }
...
static void encode_value(RGSSMarshalEncoder *e, const RGSSValue *v) {
    if (e->last_error != RGSS_MARSHAL_OK) return;
    if (!v) { enc_byte(e, '0'); return; }  // nil
    ...
    switch (v->type) {
        ...
        case RGSS_VAL_FLOAT: {
            ...
            if (idx >= 0) { enc_byte(e, '@'); enc_int(e, (long long)idx); break; }
            enc_register_obj(e, v);
            enc_byte(e, 'f');
            char fbuf[64];
            if (isinf(v->as.f))      snprintf(fbuf, sizeof(fbuf), v->as.f > 0 ? "inf" : "-inf");
            else if (isnan(v->as.f)) snprintf(fbuf, sizeof(fbuf), "nan");
            else                     snprintf(fbuf, sizeof(fbuf), "%.17g", v->as.f);
...
int rgss_marshal_dump(RGSSMarshalEncoder *e, const RGSSValue *val) {
    if (e->last_error != RGSS_MARSHAL_OK) return -1;
...
const char *rgss_marshal_error_string(RGSSMarshalError err) {
    switch (err) {
        case RGSS_MARSHAL_OK:             return "OK";
        case RGSS_MARSHAL_ERR_VERSION:    return "not a Marshal 4.8 stream";
        case RGSS_MARSHAL_ERR_TRUNCATED:  return "unexpected end of input";
        case RGSS_MARSHAL_ERR_UNSUPPORTED:return "unsupported type tag";
        case RGSS_MARSHAL_ERR_ALLOC:      return "memory allocation failed";
        case RGSS_MARSHAL_ERR_OVERFLOW:   return "integer or length overflow";
