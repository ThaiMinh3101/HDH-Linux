...
/// Sprite.new([viewport])
/// RGSS: Creates a new Sprite. Optional viewport argument (M1b: accepted but ignored).
static mrb_value
...
void
mrb_define_sprite_class(mrb_state *mrb, SpriteSetBitmapCallback bitmap_callback)
{
    g_bitmap_callback = bitmap_callback;

    struct RClass *sprite_class = mrb_define_class(mrb, "Sprite", mrb->object_class);
    MRB_SET_INSTANCE_TT(sprite_class, MRB_TT_OBJECT);

    mrb_define_method(mrb, sprite_class, "initialize", mrb_sprite_initialize, MRB_ARGS_OPT(1));
    mrb_define_method(mrb, sprite_class, "bitmap=",    mrb_sprite_set_bitmap,  MRB_ARGS_REQ(1));
    mrb_define_method(mrb, sprite_class, "bitmap",     mrb_sprite_get_bitmap,  MRB_ARGS_NONE());
}

int
mrb_bridge_run_script(mrb_state *mrb, const char *script)
{
    ...
    if (mrb->exc) {
        // Retrieve exception message before clearing exc
...
const char *
mrb_bridge_last_error(mrb_state *mrb)
{
    (void)mrb;
    return g_last_error;
}
